...
async function signUp(req, res) {
	...
	if (password !== repeatPassword) {
		return res.status(404).send({ success: false, message: "Las contraseñas tienen que ser iguales." });
	}
	...
	if (!!userEmailExists) {
		return res.status(404).send({ success: false, message: "El correo electrónico ya se encuentra registrado." });
	}
	...
	if (!!userUsernameExists) {
		return res.status(404).send({ success: false, message: "El username ya se encuentra registrado." });
	}
	...
	bcrypt.genSalt(10, function (err, salt) {
		bcrypt.hash(password, salt, function (err, hash) {
			...
			user.save((err, userStored) => {
				if (err) {
					return res.status(500).send({ success: false, message: "Error al crear el usuario: " + err });
				}
